<?php
	...
	class Userset {
		...
		function index() {
			...
			//查询用户表,用于计算访问统计
			$visited_num = D('user') -> where(array('u_id'=>$_SESSION['u_id'])) -> field('count') -> find();
			//把访问总数写到session中，方便以后调用
			$_SESSION['visited_num'] = $visited_num['count'];

			//查看关注表，用于计算访问统计
			$attention_num = D('actions') -> where(array('ucon_id'=>$_SESSION['u_id'],'type'=>1)) -> field('count') -> total();
			//计算关注量的总数写到session中
			$_SESSION['attention_num'] = $attention_num;

			//查询内容表，用于计算文章的总数
			$article_num = D('contents') -> where(array('u_id'=>$_SESSION['u_id'],'type'=>1)) -> total();
			//计算文章的总数写到session中
			$_SESSION['article_num'] = $article_num;

			//查询黑名单表,计算黑名单总数
			$blacklist_num = D('blacklist') -> where(array('u_id'=>$_SESSION['u_id'])) -> total();
			//把黑名单的总数写到session数组中
			$_SESSION['blacklist_num'] = $blacklist_num;

			//查询消息表,用于计算消息的总数
			$number=D('messages') -> where(array('gu_id'=>$_SESSION['u_id'])) -> total();
			$a=D('broadcasts') -> where(array('type'=>1)) -> total();
			$number=$number+$a;
		...
		function visited() {
			...
			$this -> assign('visited_num',$data['count']);
			$this -> assign('session',$_SESSION);
			$this -> assign('data',$data);
			$this -> display();
		}
