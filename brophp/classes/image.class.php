 ...
 * ******************************************************************************/
	class Image {
		 ...
		 */
		function __construct($path="",$newpath="./"){
			if($path=="")
				$path=PROJECT_PATH."public/uploids";
			$this->path=$path;
		}
		/**
		 * 对指定的图像进行缩放
		 ...
		 */
		function thumb($name, $width, $height,$qz="th_",$newpath,$true){
			$imgInfo=$this->getInfo($name);                                 //获取图片信息
			$srcImg=$this->getImg($name, $imgInfo);                          //获取图片资源         
			$size=$this->getNewSize($name,$width, $height,$imgInfo,$true);       //获取新图片尺寸
			$newImg=$this->kidOfImage($srcImg, $size,$imgInfo);   //获取新的图片资源
			return $this->createNewImage($newImg, $qz.$name,$imgInfo,$newpath);    //返回新生成的缩略图的名称，以"th_"为前缀
		}
		/** 
		* 为图片添加水印
		...
		*/ 
		function waterMark($groundName, $waterName, $waterPos=0, $qz="wa_",$newpath){
			...
			if(file_exists($curpath.$groundName) && file_exists($wpath.$waterName)){
				...
				return $this->createNewImage($groundImg, $qz.$groundName, $groundInfo,$newpath);
		...
		*/
  		function cut($name,$width, $height, $qz="cu_",$newpath){
			$imgInfo=$this->getInfo($name);                 //获取图片信息
			/* 裁剪的位置不能超出背景图片范围 */
			...
			if($imgInfo['width']>$imgInfo['height']){
				$y=0;
				$x=($imgInfo['width'] - $width)/2;
			}else{
				$x=0;
				$y=($imgInfo['height'] - $height)/2;
			}
			imagecopyresampled($cutimg, $back, 0, 0, $x, $y, $width, $height, $width, $height);
			imagedestroy($back);
			/* 通过本类的私有方法，保存剪切图并返回新图片的名称，默认以"cu_"为前缀 */
			return $this->createNewImage($cutimg, $qz.$name,$imgInfo,$newpath);    
		}
		...
		private function getNewSize($name, $width, $height,$imgInfo,$true){	
			...
			if($true=="ok"){
				...
				}elseif($imgInfo['width']<$imgInfo['height']){
					$size["width"] = $width;
					$size["height"] = $imgInfo["height"]*$size["width"]/$imgInfo["width"];
				}elseif($imgInfo['width']=$imgInfo['height']){
					$size["width"] = $width;
		...
		private function createNewImage($newImg, $newName, $imgInfo,$newpath){
                        $this->path=rtrim($this->path,"/")."/";
                        $this->newpath = rtrim($newpath,"/")."/";
			switch ($imgInfo["type"]) {
		   		case 1:	//gif
					$result=imageGIF($newImg, $this->newpath.$newName);
					break;
				case 2:	//jpg
					$result=imageJPEG($newImg,$this->newpath.$newName);  
					break;
				case 3:	//png
					$result=imagePng($newImg, $this->newpath.$newName);  
					break;
			}
			imagedestroy($newImg);
			return $newName;
