 ...
 * ******************************************************************************/
	class Page {
		...
		private function firstprev(){
			if($this->page > 1) {
				$str="&nbsp;<a href='javascript:getPage(\"{$this->uri}page/1\")'>{$this->config["first"]}</a>";
				$str.="&nbsp;<a href='javascript:getPage(\"{$this->uri}page/".($this->page-1)."\")'>{$this->config["prev"]}</a>&nbsp;";		
				return $str;
			}
		...
		private function pageList(){
			...
			for($i=$inum; $i>=1; $i--){
				...
				if($page>=1)
					$linkPage.="<a href='javascript:getPage(\"{$this->uri}page/{$page}\")'>{$page}</a>&nbsp;";
		...
		private function nextlast(){
			if($this->page != $this->pageNum) {
				$str="&nbsp;<a href='javascript:getPage(\"{$this->uri}page/".($this->page+1)."\")'>{$this->config["next"]}</a>&nbsp;";
				$str.="&nbsp;<a href='javascript:getPage(\"{$this->uri}page/".($this->pageNum)."\")'>{$this->config["last"]}</a>&nbsp;";
				return $str;
			}
